...
const isPerpetual=(x:Row)=>String(x["종료일"]||"").trim()==="영구";
const isExcluded=(x:Row)=>{const end=String(x["종료일"]||"").trim(),st=String(x["라이선스상태"]||"").trim();return end==="영구"||["제외","종료","만료"].includes(st);};
...
export default async function Page(){
 ...
 const renewal=d.softwareAssets.filter(x=>!isExcluded(x)&&num(x["D-DAY"])<=90).sort((a,b)=>num(a["D-DAY"])-num(b["D-DAY"]));
 ...
 <article className="panel chart"><Title t="영업기회 TOP 5" u="(단위: 건)"/><div className="ranks">{top.map(([n,c],i)=><div key={n}><b>{i+1}</b><span>{n}</span><p><i style={{width:`${c/topmax*100}%`}}/></p><strong>{c}</strong></div>)}</div></article></section>
 <article className="panel renewal"><Title t="곧 갱신 예정 고객"/><div className="table"><table><thead><tr><th>고객사</th><th>제품명</th><th>플랜</th><th>수량</th><th>종료일</th><th>D-Day</th><th>예상 금액</th><th>담당자</th><th>상태</th></tr></thead><tbody>{renewal.slice(0,8).map(x=><tr key={x["SW자산번호"]}><td>{cname(x["고객사ID"],d.customers)}</td><td>{x["제품명"]}</td><td>{x["플랜"]||"-"}</td><td>{x["수량"]||"-"}</td><td>{x["종료일"]||"-"}</td><td className={num(x["D-DAY"])<=30?"redtext":""}>{x["D-DAY"]||"-"}</td><td>{won(num(x["월금액"]))}</td><td>{x["담당자"]||"-"}</td><td><Pill v={x["라이선스상태"]}/></td></tr>)}</tbody></table></div></article>
 <section className="work"><article className="panel workcard"><h3>소프트웨어 자산 입력/수정</h3><div className="form"><label>고객사<input value={cname(cid,d.customers)} readOnly/></label><label>담당자<input value={asset["담당자"]||"-"} readOnly/></label><label>제품명<input value={asset["제품명"]||"-"} readOnly/></label><label>플랜<input value={asset["플랜"]||"-"} readOnly/></label><label>시작일<input value={asset["시작일"]||"-"} readOnly/></label><label>종료일<input value={asset["종료일"]||"-"} readOnly/></label><label>월 금액<input value={won(num(asset["월금액"]))} readOnly/></label><label>상태<input value={asset["라이선스상태"]||"-"} readOnly/></label></div><small>조회용 화면이며 수정은 구글시트에서 진행합니다.</small></article>
 <article className="panel workcard middle"><h3>고객사별 소프트웨어 현황 ({cid})</h3><div className="summary"><span>자산번호 <b>{asset["SW자산번호"]||"-"}</b></span><span>고객사 <b>{cname(cid,d.customers)}</b></span></div><table><thead><tr><th>제품명</th><th>상태</th><th>수량</th><th>만료일</th><th>비고</th></tr></thead><tbody>{cassets.map(x=><tr key={x["SW자산번호"]}><td>{x["제품명"]}</td><td><Pill v={x["라이선스상태"]}/></td><td>{x["수량"]||"-"}</td><td>{x["종료일"]||"-"}</td><td>{x["플랜"]||"-"}</td></tr>)}</tbody></table></article>
 <article className="panel workcard"><h3>영업기회 정보</h3><div className="form one"><label>기회 유형<input value={sel["기회유형"]||"-"} readOnly/></label><label>우선순위<input value={sel["우선순위"]||"-"} readOnly/></label><label>관련 고객<input value={cname(sel["고객사ID"],d.customers)} readOnly/></label><label>예상 금액<input value={won(num(sel["예상금액"]))} readOnly/></label><label>메모<textarea value={sel["메모"]||"-"} readOnly/></label></div></article></section>
 </section></main>}
