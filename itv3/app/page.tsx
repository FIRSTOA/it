...
// 수정된 부분: 종료일이 '영구' 이거나 '-' 또는 공란인 경우 제외 조건 추가
const isExcluded=(x:Row)=>{const end=String(x["종료일"]||"").trim(),st=String(x["라이선스상태"]||"").trim();return end.includes("영구")||end==="-"||end===""||["제외","종료","만료"].some(k=>st.includes(k));};
...
export default async function Page(){
 ...
 <section className="kgrid">
  ...
  <Kpi icon="▦" title="갱신 예정 (D-60)" value={`${d.softwareAssets.filter(x=>!isExcluded(x)&&num(x["D-DAY"])<=60).length}`} sub="60일 이내" tone="orange"/>
 ...
 <article className="panel renewal">
  ...
  <div className="table">
   <table>
    <thead><tr><th>고객사</th><th>제품명</th><th>플랜</th><th>수량</th><th>종료일</th><th>D-Day</th><th>예상 금액</th><th>담당자</th><th>상태</th></tr></thead>
    <tbody>{renewal.slice(0,8).map(x=><tr key={x["SW자산번호"]}><td>{cname(x["고객사ID"],d.customers)}</td><td>{x["제품명"]}</td><td>{x["플랜"]||"-"}</td><td>{x["수량"]||"-"}</td><td>{x["종료일"]||"-"}</td><td className={num(x["D-DAY"])<=30?"redtext":""}>{x["D-DAY"]||"-"}</td><td>{won(num(x["월금액"]))}</td><td>{x["담당자"]||"-"}</td><td><Pill v={x["라이선스상태"]}/></td></tr>)}</tbody>
   </table>
  </div>
 </article>
 <section className="work">
  ...
  <article className="panel workcard"><h3>영업기회 정보</h3><div className="form one"><label>기회 유형<input value={sel["기회유형"]||"-"} readOnly/></label><label>우선순위<input value={sel["우선순위"]||"-"} readOnly/></label><label>관련 고객<input value={cname(sel["고객사ID"],d.customers)} readOnly/></label><label>예상 금액<input value={won(num(sel["예상금액"]))} readOnly/></label><label>메모<textarea value={sel["메모"]||"-"} readOnly/></label></div></article>
 </section>
 </section></main>}
